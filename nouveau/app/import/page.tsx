...
// Colonnes attendues, dans l'ordre de la table equipment.
const COLONNES = [
  "id",
  "imei",
  "date",
...
// Détecte le séparateur le plus probable d'une ligne (`,` `;` ou tabulation).
...
// Découpe une ligne en tenant compte des guillemets éventuels.
  ...
  function exporterVersBase() {
    ...
    setMessage(
      `Prêt à envoyer ${lignes.length} ligne${lignes.length > 1 ? "s" : ""}. ` +
        "L'envoi vers la base sera activé une fois la connexion configurée."
    );
  }
